...
export default function LoginGoogle() {
   ...
   return (
      <form onSubmit={(e) => handleGoogleLogin(e)}>
         <button
            type="submit"
            className="flex w-64 cursor-pointer items-center justify-center rounded-full bg-transparent ring-2 transition hover:ring-4"
         >
            {!waiting ? (
               <div className="flex items-center">
                  <GmailIcon size={42} />
                  <p className="text-lg text-secondary-foreground dark:text-primary-foreground">
                     Connect with Gmail
                  </p>
               </div>
            ) : (
               ...
               >
                  <svg
                     aria-hidden="true"
                     className="h-8 w-8 animate-spin fill-primary text-gray-200 dark:text-accent"
                     viewBox="0 0 100 101"
                     fill="none"
